...
# "Inputs" heading now sits above the critical-point box (previously it
# only appeared later, inside the input column) so it labels this whole
# section — including the fluid/critical-point summary — right from the
# top of the page.
st.subheader("Inputs")

# Critical-point box sits directly under the navbar (no duplicate app-name
# title in between — the brand already shows there) and carries the Active
# Fluid line too. Values go through conv()/fmt_canon() like every other
# number in the app, so Tc/Pc update with the unit chosen below — a direct,
# always-visible T/P unit picker lives right here instead of only updating
# incidentally whenever some other T or P field elsewhere happens to have
# its own unit dropdown touched.
_cc1, _cc2 = st.columns(2)
with _cc1:
    st.selectbox("Temperature unit", _T_UNITS, key="disp_unit_T")
with _cc2:
    st.selectbox("Pressure unit", list(UNIT_DEFS["P"]["factors"].keys()), key="disp_unit_P")
try:
...
with col1:
    ...
    # want_quality's value here is just what was last chosen (from
    # session_state) — the actual checkbox widget now renders further
    # down, right next to each Quality (x) slider it controls, instead of
    # sitting alone at the top disconnected from what it affects.
    want_quality = st.session_state.get("quality_check", True) if is_two_phase_fluid else False
