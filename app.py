...
plot_config = {
    # Trimmed to just Pan + Autoscale in the modebar, plus the custom
    # fullscreen (⛶) button injected on each chart separately — box-zoom,
    # zoom-in/out, reset-axes and the download button are all removed
    # since double-click (reset+autosize) and pinch (scrollZoom) already
    # cover zooming without them.
    'displayModeBar': True,
    'responsive': True,
    'scrollZoom': True,
...
with col1:
    ...
    # ---- Step 3: critical temperature/pressure block — right after the
    # slider, nothing in between. No separate unit pickers here anymore —
    # they duplicated the "Unit" dropdown that already sits next to the
    # Temperature/Pressure field above; fmt_canon() already keeps Tc/Pc in
    # sync with that shared unit choice, so a second control was redundant.
    try:
