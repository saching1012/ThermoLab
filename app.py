...
_DARK_CSS = """
    ...
    .st-key-burger_toggle button, .st-key-wiz_prev_top button {
        ...
        padding: 0 !important;
        width: 100% !important;
        /* Fixed square footprint (not just a bigger glyph) so these read
           as real native-app icon buttons rather than oversized text,
           and comfortably clear the ~44-48px touch-target guideline. */
    ...
    /* Checkbox/radio unchecked state — same bug class as the input fields
       above: only the CHECKED state had a background set. The unchecked
       box/circle fell back to native OS rendering, which could be light
       even in app-dark-mode if the phone's OS theme is light (or the
       reverse), making the control look like a stray white/black blob. */
...
_LIGHT_CSS = """
    ...
    /* Checkbox/radio unchecked state — see the matching dark-mode comment
       above for why this needs an explicit background, not just the
       checked state. */
    div[data-testid="stCheckbox"] span[data-baseweb="checkbox"] div,
    div[data-testid="stRadio"] span[data-baseweb="radio"] div {
...
_RESPONSIVE_CSS = """
    ...
    @media (max-width: 640px) {
        ...
        .app-header .brand .em { font-size: 1.6em; }
        .app-header .brand .txt { font-size: 1.25em; }
        .app-header .brand .txt .active { font-size: 0.72em; }
        /* Plotly's own modebar (Autoscale/Reset, Download) defaults to
           tiny ~22px icons sized for a mouse — enlarge for a fingertip. */
        .modebar-btn {
...
plot_config = {
    # scrollZoom maps to pinch-to-zoom on touch, which traps a swipe meant
    # to scroll the page inside the chart instead — stays off. The modebar
    # is restored but trimmed to just Autoscale/Reset (fullscreen-equivalent
    # "reset the view" control) and Download — the mouse-oriented zoom/pan/
    # lasso tools are dropped since they don't apply to a touch-first app.
    'displayModeBar': True,
    'responsive': True,
    'scrollZoom': False,
